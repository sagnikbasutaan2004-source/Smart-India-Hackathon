...
if page == "1️⃣ Field Overview":
    ...
    if gs.get("source") in ["groq_grok", "grok", "openai"]:
        st.success(f"Generated by Grok LLM Engine ({gs.get('model')})", icon="🤖")
    else:
        st.warning("Deterministic rule-based expert engine used (no LLM key configured). Set `GROK_API_KEY` (xAI Grok) or `GROQ_API_KEY` in .env to auto-upgrade this report via GenAI on the next pipeline run.", icon="🧮")
